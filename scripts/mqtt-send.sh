...
USERNAME="${MQTT_USER}"
PASSWORD="${MQTT_PASS}"
SYSTEM_TOPIC="tie/status/system"
...
if [ "$TOPIC" = "tie/status/system" ] && [ -n "$MESSAGE" ]; then
    state="$MESSAGE"
fi

payload="$(printf '{"state":"%s","containers_running":%s,"containers_stopped":%s,"memory_usage_percent":%s,"disk_usage_percent":%s,"ssd_usage_percent":%s,"temperature":%s,"cpu_usage":%s,"uptime_seconds":%s,"boot_time":%s,"uptime":"%s"}' \
    "$state" \
    "$containers_running" \
    "$containers_stopped" \
...
if [ -n "$TOPIC" ] && [ -n "$MESSAGE" ] && [ "$TOPIC" != "$SYSTEM_TOPIC" ]; then
...
if [ "$system_status" -eq 0 ] && [ "$custom_status" -eq 0 ]; then
